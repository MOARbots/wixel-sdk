/* Wireless_serial_robot_jws app: wireless_serial + PWM on timer 3
 ...
 *
 * EXTENDED 3/3/15 By Jude Shavlik to allow a 'script' of actions to be executed after 'g' pressed
 * in Putty or screen.  Search for 'Draw a four-sided object' to see the script's code.
 */
...
uint32 currentTimeInMsec   = 0;
uint32 waitAtLeastThisLong = 0; // In msec
int32  state            = 0;
...
void main()
{
	uint8 waitForKeyPressToStart = (uint8)1;
    systemInit();
	...
	currentTimeInMsec   = getMs();
	waitAtLeastThisLong = (uint32)100; // In msec
	state            =  (int32)0;
    ...
    while(1)
    {
        ...
        {
        case SERIAL_MODE_USB_RADIO:  usbToRadioService();  break; // USB powered
        case SERIAL_MODE_USB_UART:   usbToUartService();   break; // USB *and* battery powered 
     // case SERIAL_MODE_UART_RADIO: uartToRadioService(); break; // Battery only
        case SERIAL_MODE_UART_RADIO: 
				
		T3CC0 = T3CC1 = 125; // Go slow while debugging.
		if (waitForKeyPressToStart) {  // Wait for 'g' to be pressed to start.
			if (radioComRxAvailable() && radioComRxReceiveByte() == 0x67) {
				waitForKeyPressToStart = 0;
				// turnRight();
				currentTimeInMsec = getMs();
				waitAtLeastThisLong = (uint32)250; // Wait a bit to start the 'square.'
			}
		} else if (state <= 8) { // Draw a four-sided object.  Depending on the surface's friction, this might loosely resemble a square.
			
			if ((getMs() - currentTimeInMsec) > waitAtLeastThisLong) {  // Wait at least N msec before changing actions
				switch(state) {
					case 0:
					case 2:
					case 4:
					case 6:
						T3CC0 = T3CC1 = 125; // Set the motors' duty cycles.
						goForward();
						waitAtLeastThisLong = (uint32)1500;
					break;
					case 1:
					case 3:
					case 5:
					case 7:
						T3CC0 = T3CC1 = 125;
						turnLeft();
						waitAtLeastThisLong = (uint32)333;
					break;
					case 8:
						T3CC0 = T3CC1 = 125;
						brake();
						waitAtLeastThisLong = (uint32)100;
					break;
					}
				currentTimeInMsec = getMs();  // Remember time of last change of action.
				state++;
				}
		} else { // Once the four turns made, revert to manual driving.
			T3CC0 = T3CC1 = 125;
			uartToRadioService();
		}
		ioTxSignals(radioComRxControlSignals()); // What do these do?
		radioComTxControlSignals(ioRxSignals());
		break; // End of battery only
        }
    }
}
