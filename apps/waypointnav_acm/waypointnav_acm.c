...
void robotRadioService() {
  ...
  if (!readstate) { //the idle state
      ...
      } else if (init_stage == 0) { //init stage is over
  ...
  if ( init_stage == 1 | init_stage == 2 | ( (getMs() - lastpacketRobot) > 500 ) | tagCount > (NUM_WAYPOINTS-1) ) { //if the packet is too old or we're not done with init stage or we've covered all points
    Brake(); //stop

    if (init_stage == 2) { // calculate some possibilites here
      if (numComps < 250) { // calculate 250 possible ways, done in parallel with other functions
        // shuffle(tagIDs);
        // float dist = computeTotalDistance(something here);
        // if (dist < bestDist) {
          // Replace bestOrder[]
        // }
...
//  * Found: http://stackoverflow.com/questions/1167253 */
// uint32 x=123456789,y=234567891,z=345678912,w=456789123,c=0;
// uint32 JKISS32() {
//   uint32 t;
//   y ^= (y<<5); y ^= (y>>7); y ^= (y<<22);
...
// }
//
// /* Great little function from: http://stackoverflow.com/questions/6127503 */
// void shuffle(uint8 *array, uint8 n) {
//   if (n > 1) {
//     uint8 i;
//     for (i = 0; i < n - 1; i++) {
//       uint8 j = i + rand() / (RAND_MAX / (n - i) + 1);
//       uint8 t = array[j];
//       array[j] = array[i];
//       array[i] = t;
//     }
...
float computeTotalDistance(uint8 *array, uint8 size) {
  float dist;
  uint8 i;

  for(i=0; i < (size - 1); i++) {
    dist += distance(readX(&array[i]), readY(&array[i]), readX(&array[i+1]), readY(&array[i+1]));
  }
