...
uint8 readID; //used for storing readID to figure out what queue to put stuff in
...
void usbToRadioService()
{
    ...
    uint8 signals;
    uint8 tempbyte; //for temp storage

    if (!readstate) { //the idle state
    	if (usbComRxAvailable()) { //this would change to the radio version later
   	    tempbyte = usbComRxReceiveByte(); //this would be radio version later
	    if ((tempbyte & 0xFC) == 0xFC) { //check to see the first 6 bits are 1s
    	    	readstate = 1;
	    	readID = (tempbyte | 0x03) << 3; //0 0 0 x x _ _ _, we now have the first two blanks of the ID stored in readID
    	    }
	}
	return; //don't move directly to the other state
    }
...
void uartToRadioService()
{
    ...
    uint8 i;
    XDATA Queue queue1;
    QTuple packet, result;
    InitQueue(&queue1);
    LinkQueue(&queue1, &packet); //link the queue with the Qtuple packet for input (push)
    LinkQueue(&queue1,&result); //link the queue with the Qtuple packet for output (pop)
